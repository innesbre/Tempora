...
PlotTrajectory <- function(object){
  ...
  if (length(levels(object@meta.data$Timepoints)) > 9){
    ...
    plot.igraph(edge_graph, ylim=c(-1,1), layout = l$layout, ylab = "Inferred time", vertex.shape = "pie", vertex.pie = lapply(1:nrow(object@cluster.metadata), function(x) as.numeric(object@cluster.metadata[x,levels(object@meta.data$Timepoints)])),
    ...
    plot.igraph(edge_graph, ylim=c(-1,1), ylab = "Inferred time", layout = l$layout, vertex.shape = "pie",
                vertex.pie = lapply(1:nrow(object@cluster.metadata), function(x) as.numeric(object@cluster.metadata[x,levels(object@meta.data$Timepoints)])),
